...
public class Main {
            ...
            while ((line = reader.readLine()) != null) {
                if (line.startsWith("*")) {
                    ...
                    if (command.equals("MULTI")) {
                        ...
                        byte[] response = executeCommand(elements);
                        out.write(response);
                    }
                }
            }
    ...
    private static byte[] executeCommand(String[] elements) {
        String command = elements[0].toUpperCase();
            ...
            if (command.equals("PING")) {
                ...
                if (entry != null && (entry.expiresAt() == -1 || System.currentTimeMillis() <= entry.expiresAt())) {
                    try {
                        ...
                        baos.write("-ERR value is not an integer or out of range\r\n".getBytes());
                ...
                if (idParts[1].equals("*")) {
                    ...
                    if (ms == 0 && seq == 0) {
                        baos.write("-ERR The ID specified in XADD must be greater than 0-0\r\n".getBytes());
                        return baos.toByteArray();
                    }
                }
                List<String[]> stream = streamStore.get(key);
                if (stream != null && !stream.isEmpty()) {
                    ...
                    if (ms < lastMs || (ms == lastMs && seq <= lastSeq)) {
                        baos.write("-ERR The ID specified in XADD is equal or smaller than the target stream top item\r\n".getBytes());
                        return baos.toByteArray();
                    }
                }
                ...
                baos.write("-ERR unknown command\r\n".getBytes());
            }
            return baos.toByteArray();
        } catch (IOException e) {
            return "-ERR internal error\r\n".getBytes();
